function Choi_et_al_2011_anticip(datapath,intermedpath)
% % --------------------------------------------------------------------
% This is a modification of MZ original function to read in information
% about a specifc study (see the function name).
% % --------------------------------------------------------------------
% It creates a table in which each row contains infomration from an image
% (from all participants each condition).
% The experiemental design contained 2 runs with 4 different conditions.
% The three experimental conditions and one "no stimulation" condition. 
% The design matrices look the following: There are 6 files for each
% subject. The experiment contained 2 runs, but only the results from the
% first run was published and therefore processed by MZ. We also work with
% the first run as well. The three files per runs are the following: one
% design file for the control, 1potent, and 100potent placebo. Each
% contains 14 regressors with the following:
% 1. pain anticip
% 2. first time derivative of pain anticip
% 3. pain 
% 4. first time derivative of pain 
% 5. no pain stimulation anticip (different then control)
% 6. first time derivative of no pain stimulation anticip
% 7. no pain stimulation 
% 8. first time derivative of no pain stimulation
% 9-14: most probably motion regressors
% 
% Different models were used for the estimation of control and placebo
% (only one of the conditions were in the model, while the no stimulation
% and motion were always modelled).
% 
% 
% 
% 
...
%added by BK
anticipation=zeros(size(sub));
anticipation(anti,1)=1; %0-no anticipation, 1-anticipation
...
choi.pla=pla;
choi.pain=pain;
choi.anticipation=anticipation;
