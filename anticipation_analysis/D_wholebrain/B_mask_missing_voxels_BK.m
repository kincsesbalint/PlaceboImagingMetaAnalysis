function B_mask_missing_voxels_BK(intermedpath,varargin)
...
NotconOnlywithin=find(df.contrast_imgs_only==0 & strcmp(df.study_design,'within')); % this was modified if df.contrast_imgs_only(i)==0 %...data-sets where only contrasts are available
NotconOnlybetween=find(df.contrast_imgs_only==0 & strcmp(df.study_design,'between'));
tmpdfvpainplacebo=dfv.pain_placebo;
tmpdfvpaincontrol=dfv.paincontrol;
parfor i=NotconOnlywithin % Calculate for studies with within design and nocontract only imgs
    
        
           cdata=mean(cat(3,tmpdfvpainplacebo{i},tmpdfvpaincontrol{i}),3); %if either pla or con image is nan, voxel is marked as nan
...
parfor i=NotconOnlybetween
    cdata=[tmpdfvpainplacebo{i};tmpdfvpaincontrol{i}];
    n_nan(i,:)=sum(isnan(cdata));
    n_not_nan(i,:)=sum(~isnan(cdata));
    n_subj(i,:)=size(cdata,1);
end
...
save(fullfile(intermedpath,[fname_dfv, '_masked_10_percent.mat']),'dfv_masked','-v7.3');
