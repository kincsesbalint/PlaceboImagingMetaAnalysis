function A_vectorize_all_anticip_BK(intermedpath,studyranks,varargin)
...
%todo only create studies which have anticipation phase?
dfv.anticip_pain_placebo = cell(size(df,1),1);
dfv.anticip_pain_control = cell(size(df,1),1);
...
for studyrank=studyranks
    ...
    if ex_study && exist(fullfile(intermedpath,df.anticip_placebo_minus_control{studyrank}.norm_img{1}),'file')==2
        ...
        if  ~df.contrast_imgs_only(studyrank)==1 % only vectorize where both pla and con is available.
            ...
            %with the path...
            dfv.anticip_pain_control{studyrank}=v_masked_BK(fullfile(intermedpath,curr_df_control.norm_img(validfiles_con)));
            dfv.anticip_pain_placebo{studyrank}=v_masked_BK(fullfile(intermedpath,curr_df_placebo.norm_img(validfiles_pla)));        
        end
...
if strcmp(varargin,'conservative')
    save(fullfile(intermedpath,'vectorized_images_conservative_anticip'),'dfv','-v7.3');
else
    save(fullfile(intermedpath,'vectorized_images_full_anticip'),'dfv','-v7.3');
end
end
