function C_winsorize_anticip_BK(intermedpath,varargin)
...
if strcmp(varargin,'conservative')
    load(fullfile(intermedpath,'vectorized_images_conservative_masked_10_percent.mat'),'dfv_masked')
else
...
winsorized.anticip_pain_placebo=cell(length(dfv_masked.anticip_pain_control),1);
winsorized.anticip_pain_control=cell(length(dfv_masked.anticip_pain_control),1);
winsorized.anticip_placebo_minus_control=cell(length(dfv_masked.anticip_pain_control),1);
...
if strcmp(varargin,'conservative')
    save(fullfile(intermedpath,'vectorized_images_conservative_masked_10_percent.mat'),'dfv_masked','-v7.3')
else
    save(fullfile(intermedpath,'vectorized_images_full_anticip_masked_10_percent_anticip.mat'),'dfv_masked','-v7.3')
end
end