...
if any(strcmp(varargin,'conservative'))
    ...
    load_a=load(fullfile(intermedpath,'vectorized_images_full_anticip_masked_10_percent_anticip'),'dfv_masked'); %this is the vectorized clean data (NOTE that as we exlude voxels based on all studies(from both data collection), the brainmask size differs from MZ's brain mask...)
end
...
nstudy=height(df);
for i=1:nstudy; mys.anticip_pain_control.(string(fieldnms{i}))=gpuArray(dfv_masked.anticip_pain_control{i,1});end
for i=1:nstudy; mys.anticip_pain_placebo.(string(fieldnms{i}))=gpuArray(dfv_masked.anticip_pain_placebo{i,1});end
for i=1:nstudy; mys.anticip_placebo_minus_control.(string(fieldnms{i}))=gpuArray(dfv_masked.anticip_placebo_minus_control{i,1});end
