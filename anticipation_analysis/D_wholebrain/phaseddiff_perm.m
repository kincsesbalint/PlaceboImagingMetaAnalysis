function stats=phaseddiff_perm(df,dfv,origstat,mycontrast,params,varargin)
...
if any(strcmp(mycontrast,'placebo'))
...
elseif any(strcmp(mycontrast,'control'))
    cons={'anticip_control','pain_control'};
    nonCon=find(df.contrast_imgs_only==0 & df.anticipismodeled==1);
    conOnly=[];%find(df.contrast_imgs_only==1 & df.anticipismodeled==1);
elseif any(strcmp(mycontrast,'diff'))
    cons={'anticip_placebo','pain_placebo', ...
    ...
    conOnly=find(df.contrast_imgs_only==1 & df.anticipismodeled==1);
elseif any(strcmp(mycontrast,'placebo_and_control'))
...
for studyrank=nonCon'
    ...
    if any(strcmp(varargin,'diff'))
        ...
        pla=vertcat(plac(relabel,:),contr(~relabel,:));
        con=vertcat(contr(relabel,:),plac(~relabel,:));
        stats(studyrank)=summarize_within_BK(plac,contr);
    else
        anticip=vertcat(dfv.(cons{1}).(df.study_ID{studyrank})(relabel,:),...
            dfv.(cons{2}).(df.study_ID{studyrank})(~relabel,:));
        pain=vertcat(dfv.(cons{2}).(df.study_ID{studyrank})(relabel,:),...
            dfv.(cons{1}).(df.study_ID{studyrank})(~relabel,:));
        stats(studyrank)=summarize_within_BK(anticip,pain);
    end
end
for studyrank=between_nonCon'
    nplacgr=size(dfv.(cons{1}).(df.study_ID{studyrank})-dfv.(cons{2}).(df.study_ID{studyrank}),1);
    ncongr=size(dfv.(cons{3}).(df.study_ID{studyrank})-dfv.(cons{4}).(df.study_ID{studyrank}),1);
    sample=vertcat(dfv.(cons{1}).(df.study_ID{studyrank})-dfv.(cons{2}).(df.study_ID{studyrank}),...
    dfv.(cons{3}).(df.study_ID{studyrank})-dfv.(cons{4}).(df.study_ID{studyrank})); %pool groups
...
for studyrank=conOnly'
    curr_n=size(dfv.(cons{5}).(df.study_ID{studyrank}),1);
    relabel=logical(random('Discrete Uniform',2,curr_n,1)-1); % randomly generate 0 or 1 to invert contrasts
    anticip=dfv.(cons{5}).(df.study_ID{studyrank});
    pain=dfv.(cons{6}).(df.study_ID{studyrank});
    
    pla=vertcat(anticip(relabel,:),pain(~relabel,:));
    con=vertcat(pain(relabel,:),anticip(~relabel,:));
