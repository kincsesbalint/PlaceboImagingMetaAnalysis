function B_mask_missing_voxels_anticip(intermedpath,varargin)
...
%todo save the mask in the intermediate folder not in the pubpath....

%results_path='C:\Users\lenov\Documents\PICO\C_calculations\D_results';
...
if strcmp(varargin,'conservative')
    ...
    fname_dfv='vectorized_images_full_anticip';
end
load(fullfile(intermedpath,fname_dfv),'dfv');
...
n_nan=NaN(size(df,1),size(dfv.anticip_pain_placebo{1},2));
...
for i=1:size(df,1) % Calculate for all studies except...
    if df.contrast_imgs_only(i)==0 %...data-sets where only contrasts are available
        if strcmp(df.study_design{i},'within') %Calculate for within-subject studies
           cdata=mean(cat(3,dfv.anticip_pain_placebo{i},dfv.anticip_pain_control{i}),3); %if either pla or con image is nan, voxel is marked as nan
        elseif strcmp(df.study_design{i},'between') %Calculate between-group studies
           cdata=[dfv.anticip_pain_placebo{i};dfv.anticip_pain_control{i}];
        end
    end
...
dfv_masked=dfv;
for i=1:size(df,1)
    if ~isempty(dfv.anticip_pain_placebo{i})
    dfv_masked.anticip_pain_placebo{i}=dfv.anticip_pain_placebo{i}(:,mask_exvoxels);
    end
    if ~isempty(dfv.anticip_pain_control{i})
    dfv_masked.anticip_pain_control{i}=dfv.anticip_pain_control{i}(:,mask_exvoxels);
    end
%     if ~isempty(dfv.placebo_and_control{i})
%     dfv_masked.placebo_and_control{i}=dfv.placebo_and_control{i}(:,mask_exvoxels);
%     end
    if ~isempty(dfv.anticip_placebo_minus_control{i})
    dfv_masked.anticip_placebo_minus_control{i}=dfv.anticip_placebo_minus_control{i}(:,mask_exvoxels);
    end
end
dfv_masked.brainmask=mask_exvoxels;
dfv_masked.brainmask3d=vector2img(mask_exvoxels,mask_path);

save(fullfile(intermedpath,[fname_dfv, '_masked_10_percent_anticip.mat']),'dfv_masked','-v7.3');
delete(fullfile(intermedpath,[fname_dfv,'.mat'])); %The unmasked file is huge (>2GB) and only needed once. Delete to free diskspace.

if strcmp(varargin,'conservative')
    print_image(mask_exvoxels,mask_path,fullfile(intermedpath,'conservative_masked_10_percent_anticip'))

else
    print_image(mask_exvoxels,mask_path,fullfile(intermedpath,'full_masked_10_percent_anticip'))
end
