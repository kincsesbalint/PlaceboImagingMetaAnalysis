...
%         study_dir=fullfile(df.study_dir{i},'summarized_for_meta/');
        ...
        for subjrank=1:nsubj
            ...
            outfilename{subjrank}=char(strcat(string(df.placebo{studyrank}{subjrank,"sub_ID"}),'_placebo_and_control.nii'));
            ...
            matlabbatch{subjrank}.spm.util.imcalc.expression = 'nanmean(X)';
            ...
            new_con_tbl.cond={'placebo_and_control'};
...
% (the between-group studies we add the contrast here( Kessner
% and Ruetgen!!)
i=find(strcmp(df.study_ID,'kessner'));
i_pla=cellfun(@isempty,df.control{i}.norm_img);

df.placebo_and_control{i}(~i_pla,:)=df.control{i}(~i_pla,:);
df.placebo_and_control{i}(i_pla,:)=df.pain{i}(i_pla,:);

i=find(strcmp(df.study_ID,'ruetgen'));
i_pla=cellfun(@isempty,df.control{i}.normi_img);
