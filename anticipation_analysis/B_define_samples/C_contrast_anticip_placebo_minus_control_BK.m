function C_contrast_anticip_placebo_minus_control_BK(datapath,datanewpath,intermedpath, varargin)
% % --------------------------------------------------------------------
...
% %%%
% Outputs:
% -data_frame.mat with updated anticip_placebo-control column. It contains study level
% information. In one cell, there is one table, which has columns with the keys variable names(see
% below). The table's rows contains information from one derived image
%
% It calcualtes and save the intermediate image anticipation placebo-control
% 
% 
% % --------------------------------------------------------------------
% Balint Kincses 2022
...
if ~isTableCol(df,'anticip_placebo_minus_control')
...
for studyrank=1:nstudy
    ...
    if ~strcmp((string(df.anticip_control{studyrank}.img{:})),"NA") && (~logical(df.contrast_imgs_only(studyrank))) && strcmp(df.study_design(studyrank),'within')
...
%         study_dir=fullfile(df.study_dir{i},'summarized_for_meta/');
        ...
        for subjrank=1:nsubj
            pla_df=df.anticip_placebo{studyrank}(subjrank,:);
            con_df=df.anticip_control{studyrank}(subjrank,:);
            ...
            outfilename{subjrank}=char(strcat(string(df.placebo{studyrank}{subjrank,"sub_ID"}),'_anticipation_placebo-control.nii'));
            ...
            new_con_tbl.cond={'anticipation_placebo-control'};
            ...
            if studyrank==wager_MI_studyid
                ...
                % we keep here the placebo values,as no values is provided
                % in the raw table for the anticipation part.
            ...
            df.anticip_placebo_minus_control{studyrank}(subjrank,:)=new_con_tbl;
