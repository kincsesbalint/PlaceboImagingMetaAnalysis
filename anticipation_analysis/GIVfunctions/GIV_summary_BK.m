function [summary]=GIV_summary_BK(stats, outputs,putamencorr)
...
%this is just for the mediation analyis for the putamencorrelation
         if putamencorr==1 
             [summary.ab_stand.fixed,...
         summary.ab_stand.random,...
         summary.ab_stand.heterogeneity]=GIV_weight_BK(vertcat(stats.ab_stand),vertcat(stats.se_stand_sobel));
         summary.ab_stand.n=sum(vertcat(stats(:).n),'omitnan');
         end
% Back-transform from fishersZ to r
%todo this is done now,but might not bethat important...
% [summary.r_external.fixed,...
%  summary.r_external.random,...
