function [fixed,random,heterogeneity]=GIV_weight_BK(effects,SEs)
...
% August 2010
   ...
   total_weight=sum(fixed.weight,'omitnan');
   fixed.rel_weight=fixed.weight./total_weight; %normalized weight in %
   fixed.summary=sum(effects.*fixed.weight,'omitnan')./total_weight; %Formula 7 in Deeks&Higgins
   ...
   heterogeneity.chisq=sum(fixed.weight.*(bsxfun(@minus,effects,fixed.summary)).^2,'omitnan'); % aka Q
   ...
   heterogeneity.tausq=max([(heterogeneity.chisq-fixed.df)./(total_weight-(sum(fixed.weight.^2,'omitnan')./total_weight)),
   ...
   random.rel_weight=random.weight./sum(random.weight,'omitnan');
   random.summary=sum(effects.*random.weight,'omitnan')./sum(random.weight,'omitnan'); %Formula 13 in Deeks&Higgins
   random.SEsummary=1./sqrt(sum(random.weight,'omitnan')); %Formula 14 in Deeks&Higgins
   ...
   % calculate based on Borenstein,2024 https://doi.org/10.1016/j.imr.2023.101014
   aplhalevel=0.05;
   random.predinval_lo=random.summary-tinv(1-aplhalevel/2,random.df-1).*sqrt(heterogeneity.tausq.^2+random.SEsummary.^2);
   random.predinval_hi=random.summary+tinv(1-aplhalevel/2,random.df-1).*sqrt(heterogeneity.tausq.^2+random.SEsummary.^2);
