function StudyStat=summarize_within_BK(cond1,cond2,varargin)
...
% groups is available. One can specify the aforementioned situation with
% contrast=0 and the =contrast=1, when 
%
% Based on
...
if all(size(cond1)==size(cond2)) % Check if arrays of equal size were provided
    delta=cond1-cond2;      %by-subj differences(it calculates voxeldifferences from the same row(for the nonpermuted each row is the same subject, however when we permute the order changes,thereofer we calculate values from different subject).
%     avg=(cond1+cond2)/2;
    r=fastcorrcoef_BK(cond1,cond2,'exclude_nan'); %,gpuon
    firstcontr=cond1;
    secondcontr=cond2;
    ...
    %         r(1,i)=r_raw(2);
    %     end
elseif length(cond2)==1 %contrast==1%
...
mu=mean(delta,'omitnan');      %by-study mean difference

%SD OF DIFFERENCES!
sd_diff=std(delta,'omitnan');  
se_mu=sd_diff./sqrt(n);  %by-study se (based on SD of differences!)
mean(mu./sd_diff)
mean(mu./(sd_diff./sqrt(2)))
mean(mu./(sd_diff./sqrt(2*(1-r))))
...
if ~isempty(firstcontr) || ~isempty(secondcontr)
    firstcontr(n2small)=NaN;
    secondcontr(n2small)=NaN;
end
...
% StudyStat.r_external=[];
% StudyStat.n_r_external=[];
%BK only keep those which we use later, so we might speed up things...
% StudyStat.mu=[];
% StudyStat.sd_diff=[];
% StudyStat.sd_pooled=[];
% StudyStat.se_mu=[];
...
StudyStat.r_external=[];
StudyStat.n_r_external=[];
StudyStat.pla=firstcontr;
StudyStat.contr=secondcontr;
end