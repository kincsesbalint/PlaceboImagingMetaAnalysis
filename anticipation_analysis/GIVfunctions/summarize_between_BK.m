function StudyStat=summarize_between_BK(cond1,cond2,varargin)
...
if ((length(cond1)>1) && (length(cond2)>1)) % Check if arrays were provided
    ...
    sd=sqrt((var(cond1,'omitnan').*df1+var(cond2,'omitnan').*df2)./df); %SD_pooled, aka SD_within a of cond1&2 See: [1]
